...
import { Search, Bell, Activity, Zap, Trophy, Swords, Users, Plus, Star, MapPin, Clock, ChevronRight, MessageSquare, Heart, Share2, Award, Flame } from "lucide-react";
...
const Home = () => {
        ...
        <div className="p-6 space-y-8">
          ...
          <div className="space-y-4">
            ...
            <div className="flex gap-4 overflow-hidden">
              <Skeleton className="h-40 w-60 shrink-0 rounded-3xl" />
              <Skeleton className="h-40 w-60 shrink-0 rounded-3xl" />
            </div>
          </div>
          <div className="space-y-4">
  ...
  return (
    <div className="min-h-screen bg-white pb-32 font-sans selection:bg-orange-100">
      {/* 1. TOP BAR */}
      <header className="sticky top-0 z-50 bg-white/90 backdrop-blur-xl border-b border-slate-100 px-6 h-16 flex items-center justify-between">
        <div className="flex items-center gap-2.5">
          <div className="w-9 h-9 bg-orange-600 rounded-xl flex items-center justify-center shadow-lg shadow-orange-600/20 rotate-3 hover:rotate-0 transition-transform cursor-pointer">
            <Zap className="w-5 h-5 text-white fill-current" />
          </div>
          <div className="flex flex-col leading-none">
            <span className="text-lg font-black uppercase tracking-tighter text-slate-900">RaidBook</span>
            <span className="text-[8px] font-black uppercase tracking-[0.2em] text-orange-600">Home hub</span>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <button className="p-2.5 hover:bg-slate-50 rounded-full transition-all text-slate-400 active:scale-90">
            <Search className="w-5 h-5" />
          </button>
          <button className="p-2.5 hover:bg-slate-50 rounded-full transition-all text-slate-400 relative active:scale-90 group">
          ...
          <div className="relative">
            <Avatar className="w-9 h-9 ml-1 border-2 border-slate-100 hover:border-orange-500 transition-all cursor-pointer" onClick={() => navigate('/profile')}>
              <AvatarImage src={profile?.avatar_url} />
              <AvatarFallback className="bg-slate-100 text-[10px] font-black text-slate-400 uppercase">{userName.substring(0, 2)}</AvatarFallback>
            </Avatar>
      ...
      <main className="max-w-lg mx-auto overflow-x-hidden">
        ...
        <section className="space-y-4 py-4">
          <div className="px-6 flex items-center justify-between">
            ...
            <button className="text-[10px] font-black uppercase tracking-widest text-orange-600 hover:opacity-70 transition-opacity" onClick={() => navigate('/matches')}>View All</button>
          </div>
          ...
          <div className="flex overflow-x-auto gap-4 px-6 no-scrollbar pb-4 snap-x">
            {liveMatches.map((m) => (
              <div
                key={m.id}
                className="shrink-0 w-[300px] snap-center bg-white rounded-[32px] border-2 border-slate-100 p-6 shadow-sm hover:border-orange-500/20 hover:shadow-xl hover:shadow-orange-900/5 transition-all group cursor-pointer"
                onClick={() => navigate(`/matches/${m.id}/spectate`)}
              >
                <div className="flex items-center justify-between mb-4">
                  <Badge className="bg-red-50 text-red-600 border-0 text-[9px] font-black uppercase tracking-widest h-6 px-3">Live • Half {m.current_half || 1}</Badge>
                  <div className="flex items-center gap-2">
                    <Activity className="w-3 h-3 text-slate-300" />
                    <span className="text-[10px] font-bold text-slate-400">{m.viewer_count || 124}</span>
                  </div>
                </div>
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3 flex-1 min-w-0">
                      ...
                      <span className="text-xs font-black uppercase tracking-tight text-slate-800 truncate">{m.team_a?.name || "Team A"}</span>
                    </div>
                    <span className="text-2xl font-black text-slate-900 group-hover:text-orange-600 transition-colors">{m.team_a_score}</span>
                  </div>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3 flex-1 min-w-0">
                      <div className="w-8 h-8 rounded-full bg-slate-50 border border-slate-100 flex items-center justify-center shrink-0">
                        {m.team_b?.logo_url ? <img src={m.team_b.logo_url} className="w-5 h-5 rounded-full" /> : <Users className="w-4 h-4 text-slate-300" />}
                      </div>
                      <span className="text-xs font-black uppercase tracking-tight text-slate-800 truncate">{m.team_b?.name || "Team B"}</span>
                    </div>
                    <span className="text-2xl font-black text-slate-900 group-hover:text-orange-600 transition-colors">{m.team_b_score}</span>
                  </div>
                </div>
                ...
                <div className="mt-6 pt-4 border-t border-slate-50 flex items-center justify-between">
                  ...
                  <span className="text-[10px] font-black text-orange-600 uppercase tracking-widest group-hover:translate-x-1 transition-transform">Watch Live →</span>
                </div>
              </div>
            ))}
            ...
            {upcomingMatches.map((m) => (
              <div
                key={m.id}
                className="shrink-0 w-[240px] snap-center bg-slate-50 rounded-[32px] p-6 border-2 border-transparent hover:border-blue-500/20 transition-all flex flex-col justify-between"
              >
                <div className="flex items-center justify-between">
        ...
        <section className="space-y-4 py-4">
          ...
          <div className="flex overflow-x-auto gap-4 px-6 no-scrollbar pb-4 snap-x">
            {nearbyTournaments.map((t) => (
              <div
                key={t.id}
                className="shrink-0 w-[260px] snap-center bg-white rounded-[32px] border-2 border-slate-100 overflow-hidden shadow-sm hover:border-orange-500/20 hover:shadow-xl transition-all cursor-pointer group"
              ...
              >
                <div className="h-28 bg-slate-100 relative overflow-hidden">
                  ...
                  <div className="absolute top-3 left-3">
                    <Badge className={cn(
                      ...
                      t.status === 'registering' ? "bg-green-500 text-white" : "bg-blue-500 text-white"
                    )}>
                ...
                <div className="p-5 space-y-3">
                  <h4 className="text-sm font-black uppercase tracking-tight text-slate-800 line-clamp-1 italic">{t.name}</h4>
                  <div className="space-y-1">
                    <div className="flex items-center gap-2 text-slate-400">
                      <MapPin className="w-3 h-3" />
                  ...
                  <button className="w-full py-2.5 rounded-2xl bg-slate-900 text-white text-[9px] font-black uppercase tracking-widest hover:bg-orange-600 transition-colors">
        ...
        <section className="px-6 py-6 space-y-4">
          <div className="flex items-center justify-between">
            ...
            <button className="text-[10px] font-black uppercase tracking-widest text-orange-600" onClick={() => navigate('/teams')}>Manage all</button>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            {userTeams.length > 0 ? userTeams.slice(0, 2).map((team) => (
              <div
                key={team.id}
                className="bg-white rounded-[32px] p-6 border-2 border-slate-100 shadow-sm hover:border-blue-500/20 transition-all cursor-pointer group flex flex-col items-center text-center"
                onClick={() => navigate(`/teams/${team.id}`)}
              >
                <div className="w-16 h-16 rounded-[24px] bg-slate-50 border border-slate-100 flex items-center justify-center mb-4 group-hover:scale-110 transition-transform shadow-inner">
                  {team.logo_url ? <img src={team.logo_url} className="w-10 h-10 rounded-xl" /> : <Users className="w-8 h-8 text-slate-200" />}
                </div>
                <h4 className="text-sm font-black uppercase tracking-tight text-slate-800 line-clamp-1">{team.name}</h4>
                <div className="mt-4 flex flex-col gap-1.5 w-full">
                  <div className="bg-slate-50 rounded-xl py-1.5 px-3 flex items-center justify-between">
                    <span className="text-[8px] font-black uppercase tracking-widest text-slate-400">Wins</span>
                    <span className="text-[10px] font-black text-green-600">12</span>
                  </div>
                  <div className="bg-slate-50 rounded-xl py-1.5 px-3 flex items-center justify-between">
                    <span className="text-[8px] font-black uppercase tracking-widest text-slate-400">Loss</span>
                    <span className="text-[10px] font-black text-red-600">4</span>
                  </div>
                </div>
              </div>
            )) : (
              <div className="col-span-2 bg-slate-50 rounded-[32px] p-10 flex flex-col items-center gap-4 text-center border-2 border-dashed border-slate-200 opacity-60">
                <Users className="w-10 h-10 text-slate-300" />
                <div className="space-y-1">
                  <p className="text-[10px] font-black uppercase tracking-widest text-slate-600">No teams joined</p>
                  <p className="text-[9px] font-bold text-slate-400">Build your squad to start playing</p>
                </div>
                <button
                  onClick={() => navigate('/teams')}
                  className="bg-white px-6 py-3 rounded-2xl border border-slate-200 text-[9px] font-black uppercase tracking-widest shadow-sm hover:translate-y-px transition-transform"
                >
                  Find a Team
                </button>
              </div>
        ...
        <section className="px-6 py-6 pb-20 space-y-6">
          <div className="flex items-center justify-between">
            <h3 className="text-[11px] font-black uppercase tracking-[0.25em] text-slate-400">Smart Activity</h3>
            <button className="text-[10px] font-black uppercase tracking-widest text-slate-400">Mark all read</button>
          </div>
          ...
          <div className="space-y-4">
            {activities.map((act) => (
              <div
                key={act.id}
                className={cn(
                  "bg-white rounded-[28px] p-5 border-2 border-slate-100 flex items-start gap-4 hover:border-slate-200 transition-all",
                  act.type === 'ACHIEVEMENT' && "bg-orange-50/30 border-orange-100"
                )}
              >
                <div className={cn(
                  "w-12 h-12 rounded-2xl flex items-center justify-center shrink-0 shadow-sm",
                  act.type === 'ACHIEVEMENT' ? "bg-orange-500" : "bg-slate-100"
                )}>
                  {act.type === 'ACHIEVEMENT' ? <Star className="w-5 h-5 text-white fill-current" /> : act.icon}
                </div>
                <div className="flex-1 space-y-1">
                  <div className="flex items-center justify-between">
                    <span className="text-[9px] font-black uppercase tracking-widest text-slate-400">{act.title}</span>
                    <span className="text-[8px] font-bold text-slate-300">{act.time}</span>
                  </div>
                  <p className="text-[11px] font-black text-slate-700 leading-tight uppercase tracking-tight">{act.message}</p>
                  <button className="text-[9px] font-black text-orange-600 uppercase tracking-widest pt-2">Details →</button>
                </div>
              </div>
            ))}
          </div>
